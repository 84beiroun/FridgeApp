...
//            multiIntent?.launch(intentChooser)
        ...
        binding.saveSnapButton.setOnClickListener {
    ...
    private fun selectImage() {
        ...
        alertBuilder.setItems(options, DialogInterface.OnClickListener { dialogInterface, item ->
            if (options[item] == "Take Photo") takenImage?.launch(actualImage)
            else if (options[item] == "Pick from Gallery") pickedImage?.launch(
                PickVisualMediaRequest(ActivityResultContracts.PickVisualMedia.ImageOnly)
            )
            else if (options[item] == "Cancel") dialogInterface.dismiss()
        })
        alertBuilder.show()
