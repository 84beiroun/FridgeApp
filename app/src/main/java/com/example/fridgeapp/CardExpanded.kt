...
class CardExpanded : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (fridgeSnap?.image != "null")
            binding.snapImage.setImageURI(fridgeSnap?.image?.toUri())
        else
        ...
        binding.saveSnapEditButton.setOnClickListener {
            ...
            GlobalScope.launch {
                if (binding.snapTitleOutput.text != null) {
                    val comment: String
                    if (binding.snapCommentOutput.text != null)
                        comment = binding.snapCommentOutput.text.toString()
                    else
                        comment = "null"
    ...
    companion object {
        @JvmStatic
        fun newInstance(bundlePointer: String) = CardExpanded().apply {
            arguments = Bundle().apply {
                putString(BUNDLE_POINTER, bundlePointer)
            }
        }
    }
    ...
    private fun selectImage() {
        ...
        alertBuilder.setItems(options) { dialogInterface, item ->
            when (options[item]) {
                "Take Photo" -> takenImage?.launch(actualImage)
